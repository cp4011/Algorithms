...
def stocGradAscent1(dataMatrix, classLabels):
    ...
    for j in range(40):
        ...
        for i in range(m):
            ...
            h = logistic_regression.sigmoid(sum(dataMatrix[randIndex]*weights))
            error = classLabels[randIndex].astype('float64') - h.astype('float64')          # .astype() 变量类型转换
            #print error
...
dataMat, labelMat = logistic_regression.load_dataset()
