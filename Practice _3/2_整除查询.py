...
consists of three lines. First line of each test case contains two integers N & M, second line contains N space separated
...
T = int(input())
for i in range(T):
    line1 = [int(j) for j in input().split()]
    line2 = [int(j) for j in input().split()]
    line3 = [int(j) for j in input().split()]
    N, M = line1[0], line1[1]
    for j in range(M):
        count = 0
        for k in range(N):
            if line2[k] % line3[j] == 0:
                count += 1
        if j == M-1:
            print(count)
        else:
